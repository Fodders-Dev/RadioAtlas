// Recording PR1: stream-candidate builder for the bot, copied verbatim (pure,
// dependency-free) from apps/webapp/src/lib/stationStreams.ts. The monorepo has
// no shared package (workspaces are apps/* only), so duplicating the clean
// function — and unit-testing it here — is the agreed approach. Keep in sync if
// the webapp version changes. Returns an ORDERED list of stream URLs (primary +
// fallbacks + the Salü / Radio City CDN fixes); the recorder tries them in turn.

export type StationStreamInput = {
  name: string;
...
export const buildStationStreamTargets = (station: StationStreamInput) => {
