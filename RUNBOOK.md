...
- **The edge's last `handle` does `try_files {path} /index.html`, and that matches FILES but NOT directories.** Measured 2026-08-26 against production: `/fonts/`, `/fonts` and `/globe/` all return the 5152-byte SPA shell, byte-identical to `/`, although `dist/fonts/` certainly exists. Anything that needs to be served as its own document must therefore be a file with an extension — which is why `scripts/buildStationPages.mts` writes `station/<uuid>.html` rather than `station/<uuid>/index.html`. Note the failure mode if this is ever forgotten: the SPA reads the path too, so a human lands on the right page and nothing looks wrong, while every crawler silently receives an empty shell. Clean URLs would need `try_files {path} {path}/index.html /index.html`, and Caddy here is also the edge for the neighbours' services.
- **Caddy has no MIME entry for `.webmanifest`, so it served the manifest with no `Content-Type` at all.** Measured 2026-08-26 with a raw header dump: `/manifest.webmanifest` came back `200` carrying `Cache-Control` and `Vary` and nothing else. Fixed by renaming the file to `manifest.json`, which Caddy's mime table knows AND which the existing `@appStatic` rule already names — so it now also gets the one-day cache instead of `no-store`, with no edge config change. Whether Chrome would have refused to install over the missing type was never tested; it was wrong either way, and the rename was free. `sw.js`, `og-cover.png` and the icons still fall through to the last `handle` and are served `no-store`; they work, they are just refetched.
- **`@appStatic` names three files that do not exist in `dist`.** `/catalog-fast.json` and `/catalog-full.json` both 404 (checked 2026-08-26 — there is not a single `.json` at the root of a built `dist`), and `/manifest.json` was dead config until the rename above gave it something to match. Harmless, but do not read that matcher as a description of what the site serves.
