...
- **`@appStatic` matches `/manifest.json`, but our manifest is `manifest.webmanifest`.** So the manifest, `sw.js`, `og-cover.png` and the three app icons fall through to the last `handle` and are served `no-store` instead of the one-day cache. They work — installability is unaffected — they are just refetched more than they need to be. Left alone deliberately: a few KB is not worth editing the config that fronts other people's services.
