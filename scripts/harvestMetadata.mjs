// Manual, one-shot metadata harvester (station-intelligence v1).
//
// SAFE-FIRST: gated behind HARVESTER_ENABLED=1 (default OFF), no cron / no
// resident process in this PR — the operator runs it by hand on a SMALL batch,
// we inspect coverage, THEN a cron wrapper lands as a separate PR.
//
// Run (from the repo root, with the api running on the box):
...
//
// It reads the catalog artifact, picks the top-N reachable stations by a coarse
// quality proxy, probes each via OUR /api/metadata (which already does the
// status-JSON-first + ICY + radiovanya + capped-cache work — we never touch the
// origins directly here), and writes observations + a supports_metadata flag to
// station-intelligence.sqlite. Concurrency, per-request spacing, 429/Retry-After
// and a consecutive-failure circuit breaker keep it polite and bounded; the
// one-shot exit bounds memory.
...
const main = async () => {
  ...
  log(`api=${API_BASE} catalog=${CATALOG_PATH} limit=${LIMIT} concurrency=${CONCURRENCY} pause=${PAUSE_MS}ms`);
  ...
  const batch = selectHarvestBatch(candidates, { limit: LIMIT });
  log(`selected batch: ${batch.length} reachable stations`);
  if (!batch.length) {
  ...
  if (summary.tripped) {
    log('ALERT: circuit breaker tripped — investigate upstream before re-running.');
    process.exit(2);
  }
};
