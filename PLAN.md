...
**Пасхалка Winamp** (`LitePlayerOverlay` + `winampShell`, 1220 строк) — не
удаляем, перестаём развивать.

Заморозка означает: новой работы нет, поведение не трогаем, телеметрию туда не
вкладываем. **Темы и Theme Studio морозить нельзя** — по описи они часть
архитектуры приложения, а не отдельная функция.

## Порядок

### 0.1 — Находка становится настоящей

Проверяем главное обещание: человек ловит находку и понимает, что поймал.

1. **Починить поимку.** Сегодня находка сохраняется ПОСЛЕ `await
   clipboard.writeText` и отменяется вместе с отказом буфера — молча. Сначала
   находка, потом буфер.
2. **Событие поимки** — со `stationId`, в разрешённый список API и в
   `DAILY_COUNTER_KEYS`. Без этого шага всё остальное измерять нечем.
3. **Сделать поимку видимой.** Скрытая кнопка — причина, по которой находок
   почти нет ни у кого, кроме владельца.
4. **Продолжение у находки:** открыть в своём сервисе прямо из «Треков»
   (сегодня путь обрывается на копировании текста), поделиться, спросить Лиру.
5. **Первый экран:** Лира + одна кнопка «Включай». Не анкета.
...
# История: RadioAtlas Active Roadmap
